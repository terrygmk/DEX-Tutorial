...
export const addLiquidity = async (
  ...
  try {
    ...
    // get the allowance of the user for the exchange contract
    let allowance = await tokenContract.allowance(
      signer.getAddress(),
      EXCHANGE_CONTRACT_ADDRESS
    );
    // if the allowance is less than the amount of CD tokens the user wants to add
    // then we need to approve the exchange contract to spend the tokens
    if (allowance.lt(addCDAmountWei)) {
